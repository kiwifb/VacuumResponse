...
!HPF$ DISTRIBUTE WavgA(BLOCK,BLOCK)
      ...
      double precision,dimension(0:ny-1,0:nz-1,0:nt-1,0:maxsize,nL(that),offmax,200):: actionG,CActionG
      double precision,dimension(nL(that),offmax,200)         :: avgActionG
...
c
      select case (shpe)
      case (1)
         write(suffixName,fmt='(a,a)') trim(suffixName),'.Yavg4'
         if(zero == 0) then
            write(reportName,fmt='(a,a)') trim(reportName),'.Y'
         end if

      case (2)
         write(suffixName,fmt='(a,a)') trim(suffixName),'.TYavg4'
         if(zero == 0) then
            write(reportName,fmt='(a,a)') trim(reportName),'.TY'
         end if
      case (3)
         write(suffixName,fmt='(a,a)') trim(suffixName),'.Lavg8'
         if(zero == 0) then
            write(reportName,fmt='(a,a)') trim(reportName),'.L'
         end if
      case (4)
         write(suffixName,fmt='(a,a)') trim(suffixName),'.DQ2avg4'
         if(zero == 0) then
            write(reportName,fmt='(a,a)') trim(reportName),'.DQ2'
         end if
      case (5)
         write(suffixName,fmt='(a,a)') trim(suffixName),'.DQ4avg4'
         if(zero == 0) then
            write(reportName,fmt='(a,a)') trim(reportName),'.DQ4'
         end if
      end select
c
...
c
         ...
         if(xerr == 1) then
            ...
            do icon2 = start, finish
               ...
               if(icon2 /= icon) then
                  ActionG(:,:,:,1:nYloop,:,:,icon2) = ActionG(:,:,:,1:nYloop,:,:,icon2) + ActionC(:,:,:,1:nYloop,:,:)
                  avgActionG(:,:,icon2) = avgActionG(:,:,icon2) + avgAction
               end if
...
c
      ...
      if(xerr == 1) then
         ActionG    = ActionG    / (finish - start )
         avgActionG = avgActionG / (finish - start )
      end if
...
c
                  if(xerr == 1) then
                     ...
                     do icon = 1, 200
                        ...
                        CActionG(:,:,:,iiy,Tee,offset,icon) =
     &                     actionG(:,:,:,iiy,Tee,offset,icon)/(WavgG(iiy,Tee,icon)*avgActionG(Tee,offset,icon))
                     ...
                     do ix = 0, nt-1
                        do iy = 0, nz-1
                           do iz = 0, ny-1
                              ...
                              ErrCAction(iz,iy,ix,iiy,Tee,offset) = sqrt(
     &            ((sum((CActionG(iz,iy,ix,iiy,Tee,offset,start:finish)-Caction(iz,iy,ix,iiy,Tee,offset))**2))/
...
c
         ...
         if(nodex == 1) then
            ...
            do iiy = 1, nYloop
               ...
               if(xerr == 1) then
                  write(thisconfig,fmt='(a,a,i2.2,a)') trim(reportName),'-node.',iiy,'-err.dat'
               else
                  write(thisconfig,fmt='(a,a,i2.2,a)') trim(reportName),'-node.',iiy,'.dat'
               end if
      ...
      if(nodey==1) then
         ...
         do iiy = 1, nYloop
            ...
            do ix= -nt/2, nt/2-1
               ...
               if(xerr == 1) then
                  write(thisconfig,fmt='(a,a,i2.2,a,i3.2,a)') trim(reportName),'-flux.',iiy,'x',ix,'-err.dat'
               else
                  write(thisconfig,fmt='(a,a,i2.2,a,i3.2,a)') trim(reportName),'-flux.',iiy,'x',ix,'.dat'
               end if
...
c
c  --------------------------------
c  move L and DQ shapes into the center
c  --------------------------------
c
c  L shape
c
...
c
         ...
         if(xerr == 1) then
            open (13,file=trim(reportName)//trim(fstr1)//'.err-correl',status='unknown')
         end if
c
         do Tee = 2,nL(that)
            do offset = 1,offmax
...
c
                  if(xerr == 1) then
                     write(13,'(a)')    '*********************'
                     write(13,'(a,3i3)') 'Y Loop ', iiy, Tee,offset
                     write(13,'(a)')    '*********************'
                     call fmt_writeavg(13,CAction(:,:,:,iiy,Tee,offset))
                  end if
...
c
         ...
         if(xerr == 1) then
            open (13,file=trim(reportName)//trim(fstr1)//'.err-correl.255',status='unknown')
         end if
c
...
c
                  if(xerr == 1) then
                     write(13,'(a)')    '***********************'
                     write(13,'(a,3i3)') '3-Q separation scheme ', iiy, Tee, offset
                     write(13,'(a)')    '***********************'
                  ...
                  if(dx == 1) then
                     ...
                     write(dxname,'(3a,3(i2.2))') trim(reportName),trim(fstr1),'.correl.DX-',iiy,Tee,offset

                     open (13,file=dxname,status='unknown')

                     write(dxname,'(3a,3(i2.2))') trim(reportName),trim(fstr2),'.correl.DX-',iiy,Tee,offset
                     ...
                     call fmt_writeavg(13,dxac(:,:,:))
                     call fmt_writeavg(14,dxtc(:,:,:))
