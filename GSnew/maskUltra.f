...
!------------------------------------------------------------------------------------------
!
!     Subroutine MaskUltra establishes masks for the O(a^4) improved actions
!       on lattice dimensions divisible by 4 and nlink (nlink=3 if possible)
!
...
!
      ...
      subroutine MaskUltra(mask,nlink)
...
!     mask 5 is mask 1 shifted by 1 in the x direction and so on
...
!     mask 5 is mask 1 shifted by 1 in the t direction and so on
     ...
     &        cshift( mask(:,:,:,:,4,imask+(ilink-1)*ncube), dim=4, shift=1 )
      ...
      end subroutine MaskUltra

      END MODULE GS_MASKULTRA
