...
!
!  Note: the following would have ensured the links ur0 and ui0 sat on two processors
...
!
      if ( abs(plaqbar-lastPlaq) > 1.0d-14 ) then
!
!  See if an improved action works
!
        write(*,'(a)') 'Plaquette failed: Calculating the improved action.'
        call GetAction(ur,ui,action,plaqbar,1,uzero)
        write(*,'(a,f19.16)') 'Improved action is  ', plaqbar
        write(*,'(a,f19.16)') 'Last Plaquette was  ', lastPlaq
        if ( abs(plaqbar-lastPlaq) > 1.0d-14 ) then
          nfig = 0
          write(*,'(a)') 'Configuration is corrupt.'
        else
          nfig = 2
          write(*,'(a)') 'Improved action reproduced to 14 or more places.'
        end if
      else
        nfig = 1
        write(*,'(a)') 'Plaquette action reproduced to 14 or more places.'
      end if
