!
!     cccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccccc
!     computes the product of links for the action associated with a link in the
!     xhat direction.
!
!     Authors: Frederi! D.R. Bonnet & Derek B. Leinweber
!     Last update: Derek B. Leinweber, February 2001
!
      MODULE GS_SQUARES

      CONTAINS

      subroutine squares(ur,ui,squarer,squarei,xhat,local)

      USE GS_LATTICESIZE
      USE GS_UU

      implicit none

!     global variable

      integer,parameter                                       :: nc=3
      integer,parameter                                       :: mu=4

      integer                                                 :: xhat
      logical                                                 :: local

      double precision,dimension(nx,ny,nz,nt,nc,nc)           :: squarer,squarei
!HPF$ DISTRIBUTE squarer(*,*,BLOCK,BLOCK,*,*)
!HPF$ DISTRIBUTE squarei(*,*,BLOCK,BLOCK,*,*)
      double precision,dimension(nx,ny,nz,nt,mu,nc,nc)        :: ur,ui
!HPF$ DISTRIBUTE ur(*,*,BLOCK,BLOCK,*,*,*)
!HPF$ DISTRIBUTE ui(*,*,BLOCK,BLOCK,*,*,*)

!     local variables, temporary product variables

      integer,dimension(3)                                    :: yhat
!HPF$ DISTRIBUTE yhat(*)
      double precision,dimension(nx,ny,nz,nt,nc,nc)           :: tr,ti,tsr,tsi
!HPF$ DISTRIBUTE tr(*,*,BLOCK,BLOCK,*,*)
!HPF$ DISTRIBUTE ti(*,*,BLOCK,BLOCK,*,*)
!HPF$ DISTRIBUTE tsr(*,*,BLOCK,BLOCK,*,*)
!HPF$ DISTRIBUTE tsi(*,*,BLOCK,BLOCK,*,*)
      double precision,dimension(nx,ny,nz,nt,nc,nc)           :: usqr,usqi
!HPF$ DISTRIBUTE usqr(*,*,BLOCK,BLOCK,*,*)
!HPF$ DISTRIBUTE usqi(*,*,BLOCK,BLOCK,*,*)

      integer                                                 :: imu,i
!c
!!  Timer Support
!c
!      INTEGER start_count, end_count, count_rate
!      REAL elapsed_time
!c
!!  Useage
!c
!!  CALL SYSTEM_CLOCK(start_count, count_rate)
!!  CALL SYSTEM_CLOCK(end_count)
!!  elapsed_time = REAL(end_count - start_count) / REAL(count_rate)

!     starting of the execution commands

!     setting up the yhat array
!     the yhat(1)=yhat,yhat(2)=zhat and yhat(3)=that when xhat eq 1
!     the yhat(1)=zhat,yhat(2)=that and yhat(3)=xhat when xhat eq 2
!     the yhat(1)=that,yhat(2)=xhat and yhat(3)=yhat when xhat eq 3
!     the yhat(1)=xhat,yhat(2)=yhat and yhat(3)=zhat when xhat eq 4

      do imu=1,mu-1
         yhat(imu)    = mod( xhat + imu ,4 )
         if(yhat(imu) .eq. 0) then
            yhat(imu) = 4
         end if
      end do

!      CALL SYSTEM_CLOCK(start_count, count_rate)

!     calculation of the link products in the positive plaquette.
!     starting point in the xhat direction with the mask

      squarer = 0.0d0
      squarei = 0.0d0

!     the xy,xz and xt contour when xhat eq 1
!     the yz,yt and yx contour when xhat eq 2
!     the zt,zx and zy contour when xhat eq 3
!     the tx,ty and tz contour when xhat eq 4

      do i=1,mu-1                     !loop over the yhat array
         tr = 0.0d0
         ti = 0.0d0
         tsr = cshift(ur(:,:,:,:,yhat(i),:,:),dim=xhat,shift=1)
         tsi = cshift(ui(:,:,:,:,yhat(i),:,:),dim=xhat,shift=1)
         usqr = cshift(ur(:,:,:,:,xhat,:,:),dim=yhat(i),shift=1)
         usqi = cshift(ui(:,:,:,:,xhat,:,:),dim=yhat(i),shift=1)
         call UUdag(tr,ti,tsr,tsi,usqr,usqi)

         tsr = ur(:,:,:,:,yhat(i),:,:)
         tsi = ui(:,:,:,:,yhat(i),:,:)
         call UUdag(squarer,squarei,tr,ti,tsr,tsi)

!     calculation of the link products in the negative plaquette
!     starting point in the x direction

!     the negative xy,xz and xt contour when xhat eq 1
!     the negative yz,yt and yx contour when xhat eq 2
!     the negative zt,zx and zy contour when xhat eq 3
!     the negative tx,ty and tz contour when xhat eq 4

!     .true. when we calculate the full staple
!     .false. when we calculte plaqbar where
!     only the upper 3 plaquette are needed

         if(local) then

         tr = 0.0d0
         ti = 0.0d0
         tsr = cshift(cshift(ur(:,:,:,:,yhat(i),:,:),dim=yhat(i),shift=-1),dim=xhat,shift=1)
         tsi = cshift(cshift(ui(:,:,:,:,yhat(i),:,:),dim=yhat(i),shift=-1),dim=xhat,shift=1)
         usqr = cshift(ur(:,:,:,:,xhat,:,:),dim=yhat(i),shift=-1)
         usqi = cshift(ui(:,:,:,:,xhat,:,:),dim=yhat(i),shift=-1)
         call UdagUdag(tr,ti,tsr,tsi,usqr,usqi)


         usqr = cshift(ur(:,:,:,:,yhat(i),:,:),dim=yhat(i),shift=-1)
         usqi = cshift(ui(:,:,:,:,yhat(i),:,:),dim=yhat(i),shift=-1)
         call UU(squarer,squarei,tr,ti,usqr,usqi)

         end if                    !closes the .true. if
      end do                       !closes the i=1,3 loop

!      CALL SYSTEM_CLOCK(end_count)
!      elapsed_time = REAL(end_count - start_count) / REAL(count_rate)
!c
!!  Excluding assignments
!c
!      if (local) then
!        write(*,'(/,a,f7.3,a)') 'The elapsed time is',elapsed_time,
!     &    ' seconds for 2 squares.'
!        write(*,'(a,f6.3,a)') 'The processing speed is',
!     &    2.0*1296.0*nx*ny*nz*nt/elapsed_time/1.0e09,' GFLOPS.'
!      else
!        write(*,'(/,a,f7.3,a)') 'The elapsed time is',elapsed_time,
!     &    ' seconds for 1 square.'
!        write(*,'(a,f6.3,a)') 'The processing speed is',
!     &    1.0*1296.0*nx*ny*nz*nt/elapsed_time/1.0e09,' GFLOPS.'
!      end if

      return

      end subroutine squares

      END MODULE GS_SQUARES
