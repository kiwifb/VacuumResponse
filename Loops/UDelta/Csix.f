c
c     ----------------------------------------------
c     CASE 6 - quarks at ( 9,0), (0,-5) and (0, 5)
c     ----------------------------------------------
c
c     base (bx)
c
      call product(ur,ui,ydir,cxlr,cxli, 5)

      dxlr = cshift( cxlr, dim=ydir,shift=-5)
      dxli = cshift( cxli, dim=ydir,shift=-5)

      include 'basex.f'

      call product(ur,ui,xdir,cylr,cyli,8)
      call product(ur,ui,xdir,cylr,cyli,9)
c
c     top diagonal
c
      dylr = cshift( cylr, dim=ydir,shift= 5)
      dyli = cshift( cyli, dim=ydir,shift= 5)

      tmp1r = cshift( cxlr, dim=xdir,shift= 9)
      tmp1i = cshift( cxli, dim=xdir,shift= 9)

      include 'baseup.f'
c
c     bottom diagonal
c
      dylr = cshift( cylr, dim=ydir,shift=-5)
      dyli = cshift( cyli, dim=ydir,shift=-5)

      tmp1r = cshift( dxlr, dim=xdir,shift= 9)
      tmp1i = cshift( dxli, dim=xdir,shift= 9)

      include 'basedo.f'
c
c     time links shifting
c
      do it= 1, nL(that)

         t_3r(:,:,:,:,:,:) = cshift( tlr(:,:,:,:,:,:,it), dim=ydir,shift=-5)
         t_3i(:,:,:,:,:,:) = cshift( tli(:,:,:,:,:,:,it), dim=ydir,shift=-5)

         t_1r(:,:,:,:,:,:) = cshift( tlr(:,:,:,:,:,:,it), dim=ydir,shift= 5)
         t_1i(:,:,:,:,:,:) = cshift( tli(:,:,:,:,:,:,it), dim=ydir,shift= 5)

         t_2r(:,:,:,:,:,:) = cshift( tlr(:,:,:,:,:,:,it), dim=xdir,shift= 9)
         t_2i(:,:,:,:,:,:) = cshift( tli(:,:,:,:,:,:,it), dim=xdir,shift= 9)

         include 'res.f'

         W(:,:,:,:,6,it) = Res(:,:,:,:) / 6.0d0

      end do
