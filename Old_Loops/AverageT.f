...
c
      ...
      read (*,*) start, finish
c
c  The suffix is of the form ".s 4" for example. The various Tpp, Tmm and so on will
c  be added by the program properly.
c
...
c
         ...
         open (11,file=trim(thisConfig)//trim(fstr1)//'.Tpp.correl.unf',status='old',form='unformatted')
         open (12,file=trim(thisConfig)//trim(fstr2)//'.Tpp.correl.unf',status='old',form='unformatted')
         open (13,file=trim(thisConfig)//trim(fstr1)//'.Tmm.correl.unf',status='old',form='unformatted')
         open (14,file=trim(thisConfig)//trim(fstr2)//'.Tmm.correl.unf',status='old',form='unformatted')
c
...
c
      ...
      open (9,file=trim(reportName)//'.T1.report',status='unknown')

      open (11,file=trim(reportName)//trim(fstr1)//'.T1.correl.unf',status='unknown',form='unformatted')
      open (12,file=trim(reportName)//trim(fstr2)//'.T1.correl.unf',status='unknown',form='unformatted')
...
c
      open (11,file=trim(reportName)//trim(fstr1)//'.T1.correl',status='unknown')
      open (12,file=trim(reportName)//trim(fstr2)//'.T1.correl',status='unknown')
c
      do Tee = 2,nL(that)
         do offset = 1,offmax
...
c
      open (11,file=trim(reportName)//trim(fstr1)//'.T1.correl.255',status='unknown')
      open (12,file=trim(reportName)//trim(fstr2)//'.T1.correl.255',status='unknown')
c
...
c
         write(*,'(/,3a,/)') 'Reading ',dialog,'correlations.'
         open (11,file=trim(thisConfig)//trim(fstr1)//'.Tmp.correl.unf',status='old',form='unformatted')
         open (12,file=trim(thisConfig)//trim(fstr2)//'.Tmp.correl.unf',status='old',form='unformatted')
         open (13,file=trim(thisConfig)//trim(fstr1)//'.Tpm.correl.unf',status='old',form='unformatted')
         open (14,file=trim(thisConfig)//trim(fstr2)//'.Tpm.correl.unf',status='old',form='unformatted')
c
c   avgAction and avgTopChg are recorded for each value of ixq and iyq
c   however they are independent from it. Rather than dividing the final
c   values of avgActionA and avgTopChgA by the number of time they are counted
...
c
      ...
      open (9,file=trim(reportName)//'.T2.report',status='unknown')

      open (11,file=trim(reportName)//trim(fstr1)//'.T2.correl.unf',status='unknown',form='unformatted')
      open (12,file=trim(reportName)//trim(fstr2)//'.T2.correl.unf',status='unknown',form='unformatted')
      ...
      do Tee = 2,nL(that)
         do offset = 1,offmax
            ...
            write(11) actionA(:,:,:,0:nL(xhat),:,Tee,offset)
            write(11) WavgA(0:nL(xhat),:,Tee), avgActionA(Tee,offset)
            write(12) topChgA(:,:,:,0:nL(xhat),:,Tee,offset)
            write(12) WavgA(0:nL(xhat),:,Tee), avgTopChgA(Tee,offset)
...
c  In AVS format unscaled
c
      open (11,file=trim(reportName)//trim(fstr1)//'.T2.correl',status='unknown')
      open (12,file=trim(reportName)//trim(fstr2)//'.T2.correl',status='unknown')
c
      do Tee = 2,nL(that)
         do offset = 1,offmax
...
c  Normalized to 0 ... 255
c
      open (11,file=trim(reportName)//trim(fstr1)//'.T2.correl.255',status='unknown')
      open (12,file=trim(reportName)//trim(fstr2)//'.T2.correl.255',status='unknown')
c
c  Trap preferred values
c
      do Tee = 2,nL(that)
