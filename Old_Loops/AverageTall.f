c
c  A program to average the T loops results from VacuumRespLTplan
c  or VacuumRespTpan.
...
c
         open (11,file=trim(thisConfig)//'.Tp-xy'//trim(fstr1)//'.correl.unf',status='old',form='unformatted')
         open (12,file=trim(thisConfig)//'.Tp-xy'//trim(fstr2)//'.correl.unf',status='old',form='unformatted')

         open (13,file=trim(thisConfig)//'.Tm-xy'//trim(fstr1)//'.correl.unf',status='old',form='unformatted')
         open (14,file=trim(thisConfig)//'.Tm-xy'//trim(fstr2)//'.correl.unf',status='old',form='unformatted')

         open (21,file=trim(thisConfig)//'.Tp-xz'//trim(fstr1)//'.correl.unf',status='old',form='unformatted')
         open (22,file=trim(thisConfig)//'.Tp-xz'//trim(fstr2)//'.correl.unf',status='old',form='unformatted')

         open (23,file=trim(thisConfig)//'.Tm-xz'//trim(fstr1)//'.correl.unf',status='old',form='unformatted')
         open (24,file=trim(thisConfig)//'.Tm-xz'//trim(fstr2)//'.correl.unf',status='old',form='unformatted')
c
c  ----------
c  accumulate
c  ----------
...
c
c  no need to add contribution from first T loop case (all three quarks at (0,0))
...
c
         do ix = 0, nt-1
            ...
            ActionA(1:nL(xhat),:,:,:,ix,:,:) = ActionA(1:nl(xhat),:,:,:, ix,:,:) + 
     &                                         ActionC(1:nl(xhat),:,:,:,icx,:,:)

            topChgA(1:nL(xhat),:,:,:,ix,:,:) = topChgA(1:nL(xhat),:,:,:, ix,:,:) + 
     &                                         topChgC(1:nL(xhat),:,:,:,icx,:,:)

         enddo
c
         WavgA(1:nL(xhat),:,:) = WavgA(1:nL(xhat),:,:) + Wavg(1:nL(xhat),:,:)
c
c  .Tp-xz 
c
         do ixq = 0, nL(xhat)
...
c
                     call unf_writeavg(11,actionA(ixq,iyq,:,:,:,Tee,offset),WavgA(ixq,iyq,Tee), avgActionA(Tee,offset))
                     call unf_writeavg(12,topChgA(ixq,iyq,:,:,:,Tee,offset),WavgA(ixq,iyq,Tee), avgActionA(Tee,offset))
c
         ...
         do offset = 1,offmax
            ...
            do ixq = 0, nL(xhat)
               do iyq = 0, nL(yhat)
                  ...
                  write(11,'(a)')    '*********************'
                  call fmt_writeavg(11,actionA(ixq,iyq,:,:,:,Tee,offset) / (WavgA(ixq,iyq,Tee) * avgActionA(Tee,offset)))

                  write(12,'(a)')    '*********************'
                  write(12,'(a,4i3)') 'Wilson Loop ', ixq, iyq, Tee,offset
                  write(12,'(a)')    '*********************'
                  call fmt_writeavg(12,topChgA(ixq,iyq,:,:,:,Tee,offset) / (WavgA(ixq,iyq,Tee) * avgActionA(Tee,offset)))
...
c
     ...
     &                 (maxTopChg-minTopChg)
                  ...
                  write(11,'(a)')    '*********************'
                  call fmt_writeavg(11,actionA(ixq,iyq,:,:,:,Tee,offset))

                  write(12,'(a)')    '*********************'
                  write(12,'(a,4i3)') 'Q Q-bar separation ', ixq,iyq, Tee,offset
                  write(12,'(a)')    '*********************'
                  call fmt_writeavg(11,topChgA(ixq,iyq,:,:,:,Tee,offset))
