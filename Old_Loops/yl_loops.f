...
c
      ...
      include'YLfiles/yLoopSize.h'

      CONTAINS

c
c     Create boxes of links for the Delta-shape loops
c     They are the same as for the Y shapes but we will move
c     them to point at the origin once they are computed.
...
!HPF$ DISTRIBUTE uy2i(*,*,BLOCK,BLOCK,*,*)
     ...
     &              uy1r(:,:,:,:,ic,kc)*ux2i(:,:,:,:,jc,kc) + uy1i(:,:,:,:,ic,kc)*ux2r(:,:,:,:,jc,kc)
      ...
      uy1r(:,:,:,:,:,:) = cshift( uy1r(:,:,:,:,:,:), dim = ydir, shift =-1)
      uy1i(:,:,:,:,:,:) = cshift( uy1i(:,:,:,:,:,:), dim = ydir, shift =-1)
...
c========================================================================================================
      ...
      subroutine yl_mirror(xdir,ydir,bxlr,bxli,mbllr,mblli,mbrlr,mbrli,tlr,tli,tl1r,tl1i,Res,it,xtop,ytop)
c
c     modules
c
      USE L_baryonParam
...
c
      include 'YLfiles/loop_declarations.f'
c
c     ----------------
c     execution begins
c     ----------------
...
c
      ...
      end subroutine yl_loops

c========================================================================================================

      subroutine yl_loops_mirror(xdir,ydir,ur,ui,Wp,Wm)
c
c     modules
c
      USE L_baryonParam
...
c
      Wp(:,:,:,:,5,it) = Res(:,:,:,:) / 216.0d0
c
      call delta_mirror(xdir,ydir,bxlr,bxli,bllr,blli,brlr,brli,tlr,tli,Res,it,4,-3,4)
c
      Wm(:,:,:,:,5,it) = Res(:,:,:,:) / 216.0d0
c
c     CASE 6 - quarks at ( 5,0), (-4,-5) and (-4, 5)
...
c
      Wp(:,:,:,:,6,it) = Res(:,:,:,:) / 216.0d0
c
      call delta_mirror(xdir,ydir,bxlr,bxli,bllr,blli,brlr,brli,tlr,tli,Res,it,5,-4,5)
c
      Wm(:,:,:,:,6,it) = Res(:,:,:,:) / 216.0d0
c
c     CASE 7 - quarks at ( 7,0), (-4,-6) and (-4, 6)
...
c
         Wp(:,:,:,:,8,it) = Res(:,:,:,:) / 216.0d0
c
         call delta_mirror(xdir,ydir,bxlr,bxli,bllr,blli,brlr,brli,tlr,tli,Res,it,8,-4,7)
c
         Wm(:,:,:,:,8,it) = Res(:,:,:,:) / 216.0d0
c
c
...
c
         Wp(:,:,:,:,9,it) = Res(:,:,:,:) / 216.0d0
c
         call delta_mirror(xdir,ydir,bxlr,bxli,bllr,blli,brlr,brli,tlr,tli,Res,it,9,-5,8)
c
         Wm(:,:,:,:,9,it) = Res(:,:,:,:) / 216.0d0
c
      endif
      ...
      end subroutine yl_loops_mirror

      END MODULE L_YLLOOPS
